...
This module defines the data structures for ground truth data used in
...
PITCH_SCHEMA = vol.Schema(
    {
        vol.Required("start_time"): vol.All(vol.Coerce(float), vol.Range(min=0)),
        vol.Required("end_time"): vol.All(vol.Coerce(float), vol.Range(min=0)),
        vol.Required("midi_note"): vol.All(vol.Coerce(int), vol.Range(min=0, max=127)),
        vol.Optional("frequency_hz"): vol.All(vol.Coerce(float), vol.Range(min=0)),
        vol.Optional("waveform"): vol.In(["sine", "triangle", "sawtooth", "square"]),
    }
)
...
METADATA_SCHEMA = vol.Schema(
    {
        vol.Optional("bpm"): vol.All(vol.Coerce(float), vol.Range(min=20, max=300)),
...
SIGNAL_SCHEMA = vol.Schema(
    {
        vol.Required("signal_type"): vol.In(["tempo", "onset", "pitch", "complex"]),
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        if self.ground_truth.beats:
            result["ground_truth"]["beats"] = [
                {"time": b.time, "beat_number": b.beat_number, "bar": b.bar}
                for b in self.ground_truth.beats
            ]

        if self.ground_truth.onsets:
            result["ground_truth"]["onsets"] = [
                {"time": o.time, "attack_ms": o.attack_ms, "type": o.onset_type}
